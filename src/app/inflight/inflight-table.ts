 ...
 */
export class InflightTable {
  private readonly _state: InFlightState;

  /**
   * Will yield for each state change
   */
  public $state: BehaviorSubject<InFlightState>;
   ...
   */
  public $results: BehaviorSubject<PagedResults>;
  ...
  constructor() {
    ...
    this.$state = new BehaviorSubject<InFlightState>(this._state);

    this._results = new PagedResults();
    this.$results = new BehaviorSubject<PagedResults>(this._results);

    this.$error = new Subject<Error>();
  }

  /**
   * Current value of results, it same as yielded by [$results]{@link InFlight#resultsObservable}
   *
  ...
  /**
   * Current state, it same as yielded by [$state]{@link InFlight#stateObservable}
   *
  ...
  public getPage(pageNo = 1) {
    ...
    this._pageObsSubscription = this._getPageFn(pageNo, this._perPage)
      .subscribe(
        (data) => {
          this._clearPageSubscription();
          ...
          if (data.page !== pageNo) {
            ...
            this.$error.next(new Error(`Expected page no ${pageNo} instead got page no ${data.page}`));
            return;
          }
